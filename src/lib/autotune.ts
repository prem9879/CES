...
const CONTEXT_PATTERNS: Record<ContextType, RegExp[]> = {
  code: [
    ...
    /```[\s\S]*```/,
    /\b(fix|implement|write|create|build|deploy|test|unit test|lint|npm|pip|cargo|git)\b[^]{0,200}\b(code|function|app|service|component|module)\b/i,
    /[{}();=><]/,
  ...
  creative: [
    ...
    /\b(describe|paint|envision|portray|illustrate|craft)\b[^]{0,200}\b(world|scene|character|feeling|emotion|atmosphere)\b/i,
